...
export default function Home() {
  ...
  useEffect(() => {
    const detectDevice = () => {
      ...
      // Format device name with version for specific devices
      let deviceName = os.name
      if (os.name === 'iPhone' || os.name === 'iPad') {
        // iOS version is shown as "iPhone OS 15_0" so we add the version
        deviceName = `${os.name} ${Math.floor(os.version)}`
      } else if (os.name === 'Macintosh') {
        // Simplify to MacBook
        deviceName = 'MacBook'

        // Check if it's likely a MacBook Air/Pro based on screen size
        ...
        // Windows version is shown as NT 10.0, etc.
        if (os.version >= 10) {
    ...
    for (const file of selectedFiles) {
      await sendFile(file.file, peers[0].id, (progress) => {
        ...
        setUploadProgress({
          progress,
          uploaded,
          speed: 4096 * 1024 // 4MB/s in bytes
        })
      })
    }
                ...
                <div key={`receiving-${file.name}-${index}`} className="mb-4">
                  ...
                  >
                    <Card className="w-full">
                      <CardContent className="flex flex-col gap-4 p-4 md:p-6">
                        <div className="flex items-center justify-between">
                          <div className="text-sm text-muted-foreground max-w-[150px] sm:max-w-none truncate">Downloading {file.name}...</div>
